...
export async function syncWixToHubSpot(
  ...
  try {
    ...
    // Note: wix_sync_source custom property requires crm.schemas.contacts.write scope.
    // Only include it if custom properties have been provisioned.
...
export async function syncHubSpotToWix(
  ...
  try {
    ...
    logger.info(
      "[syncHubSpotToWix] Raw HubSpot properties:",
      JSON.stringify(hsProps),
    );
